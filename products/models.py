...
class Variation(models.Model):
    """
        product : sản phẩm
        title : tiêu đề của hàng
        price : giá của hàng hóa
        sale_price : giá khi sale, giảm giá
        active : hàng này được kích hoạt , khả dụng
    ...
    def get_price(self):
        """
        trả về giá sale nếu giá sale khác None còn không thì trả về giá thật của hàng
        """
        return self.sale_price if self.sale_price else self.price

    def get_absolute_url(self):
        """trả về đường dẫn của sản phẩm"""
...
class Category(models.Model):
    """Loại sản phẩm
        ...
        slug : đường dẫn
        description : mô tả
        active trạng thái kích hoạt hay không
    """
    title = models.CharField(max_length=100)
...
class ProductFeatured(models.Model):
    """tính năng của sản phẩm
        ...
        title tiêu đề
        description mô tả
        description_right mô tả 2
        show_price có show giá k
     """
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
...
class ProductFeaturedDetail(models.Model):
